...
contract VulnerableSignatureContract {
     ...
     */
    function recoverSigner(uint8 v, bytes32 r, bytes32 s, bytes32 hash) external pure returns (address) {
     ...
     */
    function createAuthorizationHash(address user, uint256 nonce) external pure returns (bytes32) {
        bytes32 hashedVal;
        assembly {
            mstore(0x00, user)
            mstore(0x20, nonce)
